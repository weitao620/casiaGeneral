  ...
  <div class="table-classs" ref="sprintSchoolPdf">
    ...
    <div class="table-style group_01">
      ...
      <div class="table-border">
        ...
        <div class="gp_title" v-if="riskPage < 3">
          <div class="gp_head">总体风险情况</div>
          <div class="gp_txt">
            {{ partsStr2 }}
          </div>
        </div>
        <div class="gp_title" v-if="riskPage < 2">
    ...
    <div class="table-style group_01" v-if="riskPage > 7">
      <div class="table-border">
        ...
        <div style="height:10px;"></div>
        <!-- <div class="gp_title" v-if="riskPage < 8 && riskPage > 6">
          <div class="gp_exl">
            <div class="gp_con"  v-for="(item, index) in cpList3" :key="index">
              <div v-if="item.name != ''">
                {{ item.name }}：共{{ item.total }}人，实测{{ item.evaNum }}人（{{item.evaPerct }}%）
...
export default {
  ...
  computed: {
    schoolPdfs() {
      return this.$store.state.partsFlag;
    }
  },
  watch: {
  ...
  methods: {
    ...
    getMeetingAll() {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.$http
          .get(Url + "/aimw/report/exportGroupReport", { params: paramt })
          .then(res => {
            ...
            if (data.code == 0) {
              ...
              if (!info.depInfo) {
                this.$message.error('当前部门内未发现子部门的数据，请添加后再进行导出！')
                this.loading.close();
                this.setPartsFlag(false)
                return false
              }
    ...
    async exportMeeting(type) {
      ...
      try {
        ...
        for (let i = 0; i < selectedData.length; i++) {
          ...
          this.startDate = selectedData[i].startDate.substring(0, 4) + '年' + selectedData[i].startDate.substring(4, 6) + "月" + selectedData[i].startDate.substring(6, 8) + "日";
          console.log(this.startDate)
          this.endDate = selectedData[i].endDate.substring(0, 4) + '年' + selectedData[i].endDate.substring(4, 6) + "月" + selectedData[i].endDate.substring(6, 8) + "日";
          ...
          this.cpHead = '本机构共' + selectedData[i].totalNum + '人，实测' + selectedData[i].evaNum + '人（' + selectedData[i].evaPerct + '）。其中：'
          let cpList = []
          ...
          var rstr = '';
          rstr += '系统发现，本机构实测人数中有' + selectedData[i].riskNum + '人（' + selectedData[i].riskPerct + '%）存在风险，' + parseInt(selectedData[i].evaNum - selectedData[i].riskNum) + '人（' + this.perctInfo1(100 - selectedData[i].riskPerct) + "%" + '）无风险。'
            ...
            wdStr.push(
              {
                ...
                info: selectedData[i].depressionInfo
              }
            )
          }
            ...
            wdStr.push(
              {
                ...
                info: selectedData[i].anxietyInfo
              }
            )
          }
            ...
            wdStr.push(
              {
                ...
                info: selectedData[i].forcedInfo
              }
            )
          }
          if (this.ptsdFlag == 1) {
            ...
            wdStr.push(
              {
                ...
                info: selectedData[i].violenceInfo
              }
            )
          }
            ...
            wdStr.push(
              {
                ...
                info: selectedData[i].suicideInfo
              }
            )
          }
          if (this.zibiFlag == 1) {
          ...
          var wstr = '';
          wstr += '如图所示，在该测评时间段内，总体预警维度占比最多的为'
          for (let k in wdArrs) {
            if (k < 1) {
              wstr += wdArrs[k].name + '（' + wdArrs[k].perct + '%），其次分别为：'
            } else {
